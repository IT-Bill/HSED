...
int main(int argc, char *argv[]) {
    ...
    if (argc == 3) {
        ...
        if (end <= 0) {//处理start和end<=0的情况
            DoubleFunction::processNegativeRangeLayer23(input_x, ULP, start, start, intervals);
        } else if (start < 0 && end > 0) {//处理start<0但是end>0的情况，即区间跨越了0
            DoubleFunction::processCrossZeroLayer23(input_x, ULP, start, end, intervals);
        } else {//处理start和end都大于等于0的情况
            DoubleFunction::processPositiveRangeLayer23(input_x, ULP, start, end, intervals); 
        }
        // !======================================================================================
