...
def ULPErr(fx, fx_hat):
    # Calculate the error based on ULP
    fx = np.float64(fx)
    fx_hat = np.float64(fx_hat)
    ulp_fx = np.nextafter(fx, np.inf) - fx  # ULP of fx
    error_ulp = abs(fx - fx_hat) / ulp_fx
    return error_ulp
...
def calculate_high_precision(func, x, prec=128):
    ...
    try:
        result = func(x)
        if result is None:
            print(f"Warning: Function returned None for x = {x}. Handling gracefully.")
            Y_std = np.nan 
        elif isinstance(result, mpmath.mp.mpc):
            Y_std = float(result.real)  # 仅取实部
        else:
            Y_std = float(result)
    except ZeroDivisionError:
        print(f"ZeroDivisionError encountered for x = {x}. Handling gracefully.")
        Y_std = np.inf 
    return Y_std
...
if __name__ == "__main__":
    import sys
    if len(sys.argv) != 4:
        print("Usage: python script.py <func_idx> <x> <origin>")
        sys.exit(1)

    # Parse the command line arguments
    func_idx = int(sys.argv[1])
    x = float(sys.argv[2])
    origin = float(sys.argv[3])

    # Example usage of ErrULP function
    ulp_error = ULPErr(calculate_high_precision(funcDict[func_idx]["func"], x), origin)
    print(ulp_error)